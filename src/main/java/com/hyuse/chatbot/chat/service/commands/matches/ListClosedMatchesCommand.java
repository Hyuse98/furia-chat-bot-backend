...
public class ListClosedMatchesCommand extends BaseCommand {
    ...
    public boolean supports(String command) {
        return command.equalsIgnoreCase("ClosedMatches");
    }
    ...
    public ChatMessage handle(String userId) {
        ...
        try {
            List<Match> list = service.listByMatchStatus(MatchStatus.CLOSED);

            if (list.isEmpty())
                return createResponse("Nenhuma partida encontrada.");

            StringBuilder response = new StringBuilder("📅 **Partidas Encerradas:**\n\n");
            response.append("<hr>");
            list.forEach(match -> {
